...
const getPaginatedContents = async (req, res) => {
    ...
    const contents = await Content.find({}).skip(+offset).limit(+limit)
...
const deleteContent = async (req, res) => {
  ...
  if (!content)
    throw new NotFoundError(`No content with id ${contentId}`)
...
module.exports = { getContent, getPaginatedContents, deleteContent, deleteAllContents, createContent, updateContent }